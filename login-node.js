...
class UserManager {
    ...
    async createUser(user) {
        ...
        user.password = crypto.createHmac('sha256', user.salt).update(user.password).digest('hex')//encripte la contraseña que el usurio puso al momento de REGISTRARSE
    ...
    async validateUser(username, password){
        ...
        const newCrypto = crypto.createHmac('sha256', user.salt).update(password).digest('hex')//encripto la pass, que el usuario ingreso al momento de logearse
